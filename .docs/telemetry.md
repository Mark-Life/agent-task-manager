...
- Retention. The ledger is now the store you are not allowed to lose, and nothing rotates it or
  copies it anywhere. `.docs/disk.md` covers the volume; the rotation policy is unwritten.
- Sampling. At one operator's volume everything is kept, deliberately. The trigger to revisit is
  a ten-fold rise in rows per day, not a size on disk.
