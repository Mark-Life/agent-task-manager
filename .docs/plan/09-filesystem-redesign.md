...
`FilePath`. A worker gets the tree read-only plus one writable subtree; the manager gets the
tree read-write, so promotion is `mv` by whoever is looking at the file. The guardrail with a
real threat behind it, the worker's read-only parent bind, is untouched. About 6,000 lines
...
rescan at run teardown, so `artifacts_list` is stale for the length of every run, and the
shared folder is never rescanned at all. Promotion is a copy plus two rows plus a 409.
...
times at three strengths, and two of the three helpers are called `isUnder` with reversed
argument order.

**Blocks growth: yes, and this is the one the owner feels.** `mountsFor`
(`packages/sandbox/src/mounts.ts:608`) takes exactly one `projectArtifactsDir`, a task carries
...
```
/workspace                                        ro   <- files/
/workspace/projects/<pid>                         rw   <- files/projects/<pid>
...
`runTreeOf`, `RunLabels` and the second layout. The task bind disappears: the task directory
already lives inside the project bind. `SCRATCH_SEGMENT` survives unchanged for a run with no
...
`run_event`, `comment`, `agent_session`, `session_usage`, `api_key`, `audit`, the auth tables,
...
tool and no link type: it is where the read-only bind sits. The write root is the task's
project directory, or the task directory for a task with no project. The manager is the same
function with a write root of `""`, which is the only thing `managerMountsFor` ever expressed.
...
mount already grants. Writes, moves, deletes and archives stay admin.
...
- The proposal subsystem: 14 files, 3,073 lines across `packages/orchestrator`,
...
- The vocabulary: `ArtifactScope` (3 values) and `FileScope` (4 cases) from
  ...
  field of `MaterializeInput` and of the sandbox spec (`packages/sandbox/src/spec.ts:346,356`),
...
2. Move containment into `@workspace/domain`, beside `relativePathRefusalOf`, which is the one
   home the gateway, the sandbox and the dashboard all reach. `within` and `resolveUnder` from
...
7. Delete the proposal subsystem, keep the convention. The 14 files, the table and the
   ...
   it, which is what the panel was for. The eight refusal reasons and the accepted/declined
   audit row go: see "What gets worse".
8. One namespace, one layout, one mount table. This is the big one and it cannot be split,
   ...
   `MaterializeInput` and the sandbox spec (`packages/sandbox/src/spec.ts:346,356`),
   ...
   `packages/sandbox/src/history.ts:170-190` inits one repository per shared scope, so
   ...
   in `scripts/backup.ts`. Delete the dead mount points at `docker/base.Dockerfile:252`:
...
9. Move the directory when the task moves. After step 8 a task's path is computed from its
   `projectId`, and two ordinary edits strand it. `TaskPatch` carries `projectId`
   (`packages/api/src/schemas/task.ts:84`), so reparenting a card leaves
   `files/projects/<old>/tasks/<tid>` behind while every reader resolves the new path. Deleting
   a project nulls `project_id` instead of cascading (`packages/db/src/schema/task.ts:85`, and
   `ProjectRepo.delete` says so at `packages/db/src/repositories/project.ts:186`), so its tasks
   all resolve to `files/tasks/<tid>` with their files under a project that is gone. Both paths
   `mv` the directory in the transaction that writes the row — same filesystem, refuse if the
   destination exists. Without this, step 8 loses files on an edit nobody reads as destructive.
10. Skills onto the same vocabulary. `packages/api/src/groups/skills.ts` (170) drops
...
11. Add the verbs and the free directory. `GET /files/archive?path=` streaming
    `tar -czf - -C <root> <path>` with fixed argv and no shell, containment resolved before
    the spawn, invocation copied from `scripts/backup.ts`. Multipart upload and download on
    the same family, one 64 MiB cap. `apps/dashboard/src/features/files/editor.tsx` stops
    pinning `encoding: "utf8"` and offers a binary as a download instead of refusing it.
    `packages/orchestrator/src/seed.ts` creates `files/notes/`.
12. Correct the record. `.docs/plan/08-agent-filesystem.md`: drop the vault section (no code
...
A worker can write its whole project directory, which now contains its sibling tasks' folders.
Today those folders live elsewhere and are unreachable. A prompt injection in a cloned README
can overwrite another task's evidence. The reversal is to bind the project directory read-only
and the task directory read-write, which costs the property that a run leaves a document the
next task in the project reads.
...
The proposal audit trail goes. Git records what landed, never what a run asked for and which
of the eight refusal reasons declined it. Pending proposals that were never decided are gone
at the migration.
...
The dashboard decorates the rows from `apps/dashboard/src/api/board-cache.ts`.
...
The archive route spawns a subprocess from the gateway, the process holding the GitHub token,
the database credentials and every agent-home login. First one in that process, fixed argv
against a fixed binary, and a new class of bug there.
...
1. Sibling task folders: accept one read-write bind at the project directory, or isolate with
   a read-only project bind plus a read-write task bind and give up a run writing project
   material?
2. Is a proposal file sitting in a task folder enough of a record, or does the accepted /
   declined decision keep a row?
3. Project directories as `projects/<projectId>`, or as a slug that reads well in a shell and
   needs a unique constraint plus a directory move on rename?
4. Does search ship as a route running `rg` from the gateway, given the archive route already
   opens that door, or does it wait?
