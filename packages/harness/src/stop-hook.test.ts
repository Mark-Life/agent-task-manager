...
describe("decideStop", () => {
  ...
  test("asks for a short comment rather than three topics at length", () => {
    ...
    expect(NO_COMMENT_REFUSAL).toContain("Keep it short");
    expect(NO_COMMENT_REFUSAL).toContain("rather than restating it");
  });
});
