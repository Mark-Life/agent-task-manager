...
export const MANAGER_SERVER_NAME = "atm";

/** What the board tools are called once a provider namespaces them. */
export const MANAGER_TOOL_PREFIX = `mcp__${MANAGER_SERVER_NAME}__`;

/** The runtime that runs the bundle. Resolved on `PATH` — every image here pins bun. */
export const MANAGER_MCP_COMMAND = "bun";

/** What launching this server takes, once the bundle is on a mount the container can read. */
export interface ManagerMcpStdio {
  /** The bundle's path **inside the container**, e.g. `/run/manager-mcp.js`. */
...
const stdioEnv = (server: ManagerMcpStdio) => ({
 ...
 */
export const managerMcpServersFile = (server: ManagerMcpStdio) => ({
