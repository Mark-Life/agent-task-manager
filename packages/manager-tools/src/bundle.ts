...
export const MANAGER_MCP_SEGMENT = "bin";

/** The bundle's file name, on the host and inside the container alike. */
export const MANAGER_MCP_BUNDLE_FILE = "manager-mcp.js";

/** The bundle on the host, under the data root. The build script writes exactly here. */
export const managerMcpBundlePathOf = (dataRoot: string) =>
  join(dataRoot, MANAGER_MCP_SEGMENT, MANAGER_MCP_BUNDLE_FILE);
 ...
 * account for.
 */
export const CONTAINER_MANAGER_MCP_PATH = join(
  CONTAINER_RUN_DIR,
  MANAGER_MCP_BUNDLE_FILE
);

/** The bundle inside one thread's run directory on the host, which is what gets copied. */
export const managerMcpRunCopyPathOf = (threadRunDir: string) =>
  join(threadRunDir, MANAGER_MCP_BUNDLE_FILE);
