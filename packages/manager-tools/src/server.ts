...
import { MANAGER_SERVER_NAME } from "./provider-config";
 ...
 */
export const MANAGER_MCP_VERSION = "1.0.0";

/** One tool as an MCP client is told about it. */
const listedTool = (tool: (typeof MANAGER_TOOLS)[number]) => ({
 ...
 */
export const callManagerTool = (options: {
  readonly args: unknown;
  readonly client: GatewayClient;
  readonly name: string;
}) => {
  const tool = managerToolByName(options.name);
  if (tool === undefined) {
    return Effect.succeed(
      toolResult({
        isError: true,
        text: `no such tool: ${options.name}. Available: ${MANAGER_TOOLS.map((each) => each.name).join(", ")}`,
      })
    );
  }
...
export const makeManagerMcpServer = (client: GatewayClient) => {
  const server = new Server(
    { name: MANAGER_SERVER_NAME, version: MANAGER_MCP_VERSION },
    { capabilities: { tools: {} } }
  );
  server.setRequestHandler(ListToolsRequestSchema, () =>
    Promise.resolve({ tools: MANAGER_TOOLS.map(listedTool) })
  );
